...
class Abonent:
    ...
    def info(self):
        return (self.ID, self.surname, self.name, self.middlename, self.address, self.credit_num, self.time_out_city, self.time_in_city)
...
def first(mas):
    ...
    for _ in range(10):
        ...
        students.append(temp.characters(person.surname() + ' ' + string[randint(0, len(string) - 1)] + '.' + string[randint(0, len(string) - 1)], 'ВКБ22', [randint(3, 5) for i in range(5)]))
...
def second(mas):
    ...
    for _ in range(5):
        temp = Train()
        trains.append(temp.characters(mimesis.Address(locale=mimesis.Locale.RU).city(), randint(1, 10), str(mimesis.Datetime().time().hour).zfill(2) + ':' + str(mimesis.Datetime().time().minute).zfill(2)))
...
def third(mas):
    ...
    for string, value in commands.items():
        if re.fullmatch(string, mas): return [', '.join(list(str(i) for i in value()))]
...
def fourth(mas):
    ...
    elif re.fullmatch(r'Найти книгу по автору ([\d\-\.A-Яа-яA-Za-z\s]+)', mas): return books.find_author(' '.join(mas.split(' ')[4:]))
    elif re.fullmatch(r'Найти книгу по названию ([\d\-\.A-Яа-яA-Za-z\s]+)', mas): return books.find_title(' '.join([str(i) for i in mas.split(' ')[4:]]))
    elif re.fullmatch(r'Найти книгу по году (\d+)', mas): return books.find_age(' '.join(mas.split(' ')[4:]))
    else:
        for string, value in commands.items():
...
def fifth(mas):
    ...
    for _ in range(10):
        ...
        temp = Customer(person.surname(), person.name(), ru.patronymic(gender=(mimesis.Gender.MALE, mimesis.Gender.FEMALE)[randint(0, 1)]), \
...
def abonent_inter_city(abon, mas):
    ans = []
    for i in abon:
...
def sixth(mas):
    ...
    for _ in range(10):
        person = mimesis.Person('ru')
        ru = RussiaSpecProvider()
        temp = Abonent(randint(1, 1000), person.surname(), person.name(), ru.patronymic(gender=(mimesis.Gender.MALE, mimesis.Gender.FEMALE)[randint(0, 1)]), \
