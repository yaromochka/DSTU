...
<br><b>Определение 1.8.</b> Цепочка β ∈ ($V^{T}$ ∪ $V^{N}$)* выводима из цепочки α ∈ ($V^{T}$ 
∪ $V^{N}$)+ в грамматике G = ($V^{T$, $V^{N}$, P, S) (обозначается α⇒*β), если 
существует последовательность цепочек $γ_{0}$, $γ_{1}$, K, $γ_{n}$ (n ≥ 0) такая, что $α = γ_{0} ⇒ γ_{1} ⇒ K ⇒ γ n = β$.
